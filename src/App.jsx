...
// localStorage shim — same API as storage used in the Claude artifact
const storage = {
  get: async (key) => {
...
function Flag({ team, size=24 }) {
  ...
  const em = FLAGS[team]||"🏳";
  return (
    <span style={{ width:size,height:size,borderRadius:"50%",background:bg,display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:size*0.5,flexShrink:0,border:"1.5px solid rgba(0,0,0,0.1)" }}>
      {em}
    </span>
  );
}
...
export default function App() {
  ...
  const [admin, setAdmin] = useState({ brazil:{}, groups:{}, knockout:{}, knockoutUnlocked:false });
  ...
  async function load() {
    try { const p=await storage.get("parts2026"); if(p) setParts(JSON.parse(p.value)); } catch{}
    try { const r=await storage.get("admin2026"); if(r) setAdmin(JSON.parse(r.value)); } catch{}
  }
  async function saveParts(d) { try{ await storage.set("parts2026",JSON.stringify(d)); }catch{} }
  async function saveAdmin(d) { try{ await storage.set("admin2026",JSON.stringify(d)); }catch{} }

  function totalFor(name) {
    const p=parts[name]; if(!p) return 0;
    let t=0;
    BRAZIL_GAMES.forEach(g=>{ const x=calcBrazilPts(p.brazil[g.id]||{},admin.brazil[g.id]); if(x!=null) t+=x; });
    GROUPS.forEach(g=>{ const x=calcGroupPts(p.groups[g.id]||{},admin.groups[g.id]); if(x!=null) t+=x; });
    t+=calcKOPts(p.knockout,admin.knockout);
    return t;
  }
  ...
  function enter() {
    ...
    const up={...parts};
    if(!up[name]) up[name]={ name, brazil:{}, groups:{}, knockout:{} };
    setParts(up); saveParts(up); setUser(name); setScreen("guesses");
  }

  const sorted = Object.keys(parts).sort((a,b)=>totalFor(b)-totalFor(a));

  // ── user palpites update helpers ───────────────────────────────────────────
  function updBrazil(gId,val) {
    const up={...parts,[user]:{...parts[user],brazil:{...parts[user].brazil,[gId]:val}}};
    setParts(up);
  }
  function updGroup(gId,field,team) {
    const p=parts[user];
    const up={...parts,[user]:{...p,groups:{...p.groups,[gId]:{...(p.groups[gId]||{}),[field]:team}}}};
    setParts(up);
  }
  function updKO(rId,idx,team) {
    const p=parts[user];
    const slots=[...(p.knockout[rId]||[])]; slots[idx]=team||null;
    // clear downstream
    const ri=KO_ROUNDS.findIndex(r=>r.id===rId);
    let newKO={...p.knockout,[rId]:slots};
    let ix=idx;
    ...
    const up={...parts,[user]:{...p,knockout:newKO}};
    setParts(up);
  }
  async function saveGuesses() {
    setSaveStatus("saving"); await saveParts(parts); setSaveStatus("saved");
    setTimeout(()=>setSaveStatus(""),2000);
  }

  // ── render ─────────────────────────────────────────────────────────────────
  if (screen==="home") return (
    <HomeScreen nameInput={nameInput} setNameInput={setNameInput} onEnter={enter}
      sorted={sorted} getTotal={totalFor}
      onAdmin={()=>setScreen("admin-login")} onRanking={()=>setScreen("ranking")}
  ...
  if (screen==="admin") return (
    <AdminPanel results={admin} setResults={setAdmin}
      onSave={async r=>{ await saveAdmin(r); setSaveStatus("saved"); setTimeout(()=>setSaveStatus(""),2000); }}
      saveStatus={saveStatus} onBack={()=>setScreen("home")} />
  );
  if (screen==="ranking") return (
    <RankingScreen sorted={sorted} getTotal={totalFor} admin={admin} parts={parts}
      onBack={()=>setScreen("home")} onSelect={n=>{setUser(n);setScreen("guesses");}} />
  );
  if (screen==="guesses") {
    const p=parts[user]; if(!p) return null;
    return <GuessesScreen user={user} p={p} admin={admin}
      onBrazil={updBrazil} onGroup={updGroup} onKO={updKO}
      onSave={saveGuesses} saveStatus={saveStatus} onBack={()=>setScreen("home")} />;
  }
  return null;
}

// ── home ──────────────────────────────────────────────────────────────────────
function HomeScreen({ nameInput, setNameInput, onEnter, sorted, getTotal, onAdmin, onRanking, onRules }) {
  return (
    <div style={{ maxWidth:480,margin:"0 auto",padding:"2rem 1rem" }}>
      ...
      <div style={{ textAlign:"center",marginBottom:"2rem" }}>
        ...
        <div style={{ display:"inline-flex",alignItems:"center",gap:6,marginBottom:6 }}>
          ...
          <h1 style={{ margin:0,fontSize:26,fontWeight:500,letterSpacing:"-0.02em" }}>Bolão Copa 2026</h1>
          <span style={{ fontSize:22 }}>🇧🇷</span>
        </div>
        <div style={{ display:"inline-block",background:"#009c3b",color:"#fff",borderRadius:"var(--border-radius-md)",padding:"3px 14px",fontSize:13,fontWeight:500,letterSpacing:"0.04em",marginBottom:8 }}>
          FAMÍLIA LINDA
        </div>
      ...
      <div style={{ background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"1.25rem",marginBottom:"1rem" }}>
        <label style={{ fontSize:13,color:"var(--color-text-secondary)",display:"block",marginBottom:8 }}>Seu nome</label>
        <div style={{ display:"flex",gap:8 }}>
          <input type="text" placeholder="Ex: Thiago" value={nameInput}
            ...
            style={{ flex:1 }} />
          <button onClick={onEnter} style={{ padding:"0 18px",whiteSpace:"nowrap" }}>
            {sorted.includes(nameInput.trim())?"Editar":"Entrar"} <i className="ti ti-arrow-right" />
          </button>
        </div>
      </div>
      ...
      {sorted.length>0 && (
        <div style={{ background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"1.25rem",marginBottom:"1rem" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
            <span style={{ fontSize:14,fontWeight:500 }}>Placar parcial</span>
          ...
          {sorted.slice(0,6).map((name,i)=>(
            <div key={name} style={{ display:"flex",alignItems:"center",gap:10,padding:"5px 0",borderTop:i>0?"0.5px solid var(--color-border-tertiary)":"none" }}>
              <span style={{ fontSize:i<3?16:13,width:24,textAlign:"center" }}>{i===0?"🥇":i===1?"🥈":i===2?"🥉":`#${i+1}`}</span>
              <span style={{ flex:1,fontSize:14 }}>{name}</span>
              <span style={{ fontSize:14,fontWeight:500 }}>{getTotal(name)} pts</span>
            </div>
          ))}
      ...
      <div style={{ display:"flex",gap:8,marginTop:4 }}>
        <button onClick={onRules} style={{ flex:1,background:"none",border:"0.5px solid var(--color-border-tertiary)",color:"var(--color-text-secondary)",fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",gap:6 }}>
          <i className="ti ti-info-circle" /> Regras de pontuação
        </button>
...
function RulesScreen({ onBack }) {
  const sections = [
    {
      emoji:"🇧🇷",
      title:"Jogos do Brasil — Fase de Grupos",
      subtitle:"Palpite de placar para cada um dos 3 jogos do Brasil",
      rows:[
    ...
    },
    {
      ...
      subtitle:"Palpite de 1º e 2º colocado para cada um dos 12 grupos",
      rows:[
        { pts:5, icon:"🥇", label:"Acertou a posição exata",    desc:'Marcou o time certo no 1º ou no 2º lugar' },
        { pts:2, icon:"🎖️", label:"Acertou entre os dois primeiros", desc:'O time que você escolheu passou, mas na posição errada' },
    ...
    },
    {
      emoji:"🏟️",
      title:"Mata-mata",
      subtitle:"Palpite de quem avança em cada fase. Preenchido após o chaveamento ser definido.",
      rows:[
        { pts:5,  icon:"⚔️", label:"Rodada de 32",  desc:'Acertou quem avançou de cada partida' },
        { pts:10, icon:"⚔️", label:"Oitavas de final", desc:'Acertou quem avançou de cada partida' },
        { pts:15, icon:"⚔️", label:"Quartas de final", desc:'Acertou quem avançou de cada partida' },
        { pts:20, icon:"⚔️", label:"Semifinais",       desc:'Acertou quem avançou de cada partida' },
        { pts:60, icon:"🏆", label:"Campeão",          desc:'Acertou o campeão da Copa do Mundo 2026' },
      ],
      note:"Os palpites do mata-mata ficam bloqueados até o chaveamento oficial ser divulgado."
    },
  ];
  ...
  return (
    <div style={{ maxWidth:560,margin:"0 auto",padding:"1.5rem 1rem" }}>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:"1.5rem" }}>
        ...
        <div>
          <p style={{ margin:0,fontSize:12,color:"var(--color-text-secondary)" }}>Bolão Família Linda</p>
      ...
      {sections.map((s,si)=>(
        <div key={si} style={{ marginBottom:"1.5rem" }}>
          ...
          <div style={{ background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",overflow:"hidden" }}>
            {s.rows.map((row,ri)=>(
              <div key={ri} style={{ display:"flex",alignItems:"center",gap:12,padding:"10px 16px",borderTop:ri>0?"0.5px solid var(--color-border-tertiary)":"none" }}>
                {/* points badge */}
                <div style={{
                  width:42,height:42,borderRadius:"var(--border-radius-md)",flexShrink:0,
                  background: row.pts===0?"var(--color-background-secondary)":row.pts>=20?"#009c3b22":row.pts>=10?"var(--color-background-warning)":"var(--color-background-info)",
                  display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"
                }}>
                  <span style={{ fontSize:10,fontWeight:500,
                    color: row.pts===0?"var(--color-text-tertiary)":row.pts>=20?"#009c3b":row.pts>=10?"var(--color-text-warning)":"var(--color-text-info)"
                  }}>{row.pts}</span>
          ...
          {s.note && (
            <p style={{ margin:"8px 0 0",fontSize:12,color:"var(--color-text-secondary)",padding:"0 4px" }}>
              ℹ️ {s.note}
            </p>
          )}
        </div>
      ...
      <div style={{ background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",overflow:"hidden",marginBottom:"1.5rem" }}>
        <div style={{ padding:"10px 16px",background:"var(--color-background-secondary)",borderBottom:"0.5px solid var(--color-border-tertiary)" }}>
          <p style={{ margin:0,fontWeight:500,fontSize:14 }}>Resumo por categoria</p>
        </div>
        {[
          { label:"Brasil (3 jogos)", max:3*20, note:"máx 20pts cada" },
          { label:"Grupos (12 grupos)", max:12*10, note:"máx 10pts cada" },
          { label:"Rodada de 32 (16 partidas)", max:16*5, note:"5pts cada" },
          { label:"Oitavas (8 partidas)", max:8*10, note:"10pts cada" },
          { label:"Quartas (4 partidas)", max:4*15, note:"15pts cada" },
          { label:"Semifinais (2 partidas)", max:2*20, note:"20pts cada" },
          { label:"Final — Campeão", max:60, note:"60pts" },
        ].map((row,ri)=>(
          <div key={ri} style={{ display:"flex",alignItems:"center",padding:"8px 16px",borderTop:ri>0?"0.5px solid var(--color-border-tertiary)":"none" }}>
            <span style={{ flex:1,fontSize:13 }}>{row.label}</span>
            <span style={{ fontSize:11,color:"var(--color-text-tertiary)",marginRight:12 }}>{row.note}</span>
            <span style={{ fontSize:13,fontWeight:500,minWidth:48,textAlign:"right" }}>até {row.max}pts</span>
          </div>
        ))}
        <div style={{ display:"flex",alignItems:"center",padding:"10px 16px",borderTop:"0.5px solid var(--color-border-tertiary)",background:"var(--color-background-secondary)" }}>
          <span style={{ flex:1,fontSize:14,fontWeight:500 }}>Total máximo</span>
          <span style={{ fontSize:16,fontWeight:500 }}>{maxPts} pts</span>
        </div>
      </div>
...
// ── admin panel ───────────────────────────────────────────────────────────────
function AdminPanel({ results, setResults, onSave, saveStatus, onBack }) {
  ...
  return (
    <div style={{ maxWidth:720,margin:"0 auto",padding:"1.5rem 1rem" }}>
      <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:"1.5rem" }}>
        ...
        <button onClick={()=>onSave(results)} style={{ marginLeft:"auto",padding:"7px 16px" }}>
...
function AdminKO({ results, setResults, onSave }) {
  ...
  // official result for a match: toggle pick or clear downstream
  function setOfficialWinner(roundId, matchIdx, team) {
    ...
    // clear downstream official results
  ...
  // teams entering each official round
  ...
  const SUB = [{id:"bracket",label:"Chaveamento R32"},{id:"results",label:"Resultados por fase"},{id:"vis",label:"Visibilidade"}];

  return (
    <div>
      <TabBar tabs={SUB} active={sub} onChange={setSub} />

      {/* ── Chaveamento R32: define the 16 matches ── */}
      {sub==="bracket" && (
        <div>
          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:12,padding:"8px 12px",background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)" }}>
            <span style={{ fontSize:13,color:"var(--color-text-secondary)" }}>
              Confrontos preenchidos: <strong style={{ color:"var(--color-text-primary)" }}>{Math.floor(filledEntrants/2)}/16</strong>
            </span>
            {filledEntrants===32 && <span style={{ fontSize:12,color:"var(--color-text-success)",fontWeight:500 }}>✓ Completo</span>}
          </div>
          <p style={{ fontSize:13,color:"var(--color-text-secondary)",marginBottom:12 }}>
            Selecione as duas seleções de cada partida da Rodada de 32.
          </p>
                ...
                <div key={mi} style={{ background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"10px 14px" }}>
                  ...
                  {[i1,i2].map((idx,side)=>(
                    <div key={idx} style={{ display:"flex",alignItems:"center",gap:8,marginBottom:side===0?6:0 }}>
                      {entrants[idx]
                        ? <Flag team={entrants[idx]} size={22} />
                        : <span style={{ width:22,height:22,borderRadius:"50%",background:"var(--color-background-secondary)",display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"var(--color-text-tertiary)",flexShrink:0 }}>?</span>
                      }
      ...
      {/* ── Resultados: register winners per round ── */}
      {sub==="results" && (
        <div>
          <p style={{ fontSize:13,color:"var(--color-text-secondary)",marginBottom:16 }}>
            Clique no vencedor de cada partida para registrar o resultado oficial.
            Clique novamente para desfazer. Isso atualiza a pontuação de todos automaticamente.
          </p>
          {KO_ROUNDS.map((round,ri)=>{
            const teams=officialTeams(round.id);
            const winners=ko[round.id]||[];
            const numM=round.slots/2;
            const prevComplete = ri===0
              ? filledEntrants===32
              : (ko[KO_ROUNDS[ri-1].id]||[]).filter(Boolean).length===KO_ROUNDS[ri-1].slots/2;
            ...
            return (
              <div key={round.id} style={{ marginBottom:24 }}>
                <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:10 }}>
                  ...
                  <span style={{ fontSize:12,color:doneCount===numM?"var(--color-text-success)":"var(--color-text-secondary)" }}>
                    {doneCount}/{numM} definidos
                  </span>
                  {!prevComplete && ri>0 && <span style={{ fontSize:12,color:"var(--color-text-warning)" }}>⚠ fase anterior incompleta</span>}
                </div>
                    ...
                    return (
                      <div key={mi} style={{ background:"var(--color-background-primary)",border:winner?"1.5px solid var(--color-border-success)":"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",overflow:"hidden" }}>
                        <div style={{ padding:"5px 10px 3px",fontSize:11,color:"var(--color-text-tertiary)",fontWeight:500,background:"var(--color-background-secondary)" }}>
                          {round.short} #{mi+1} {winner&&<span style={{ color:"var(--color-text-success)" }}>✓</span>}
                        </div>
                        {[t1,t2].map((team,ti)=>{
                          const isW=winner===team;
                          const bg=COLORS[team]||"#888";
                          return (
                            <div key={ti} onClick={()=>team&&setOfficialWinner(round.id,mi,team)} style={{
                              display:"flex",alignItems:"center",gap:8,padding:"8px 10px",
                              cursor:team?"pointer":"default",
                              background:isW?bg+"22":"transparent",
                              borderTop:ti>0?"0.5px solid var(--color-border-tertiary)":"none",
                              transition:"background 0.12s"
                            }}>
                              {team ? (
                                <>
                                  <Flag team={team} size={24} />
                                  <span style={{ fontSize:13,flex:1,fontWeight:isW?500:400 }}>{team}</span>
                                  {isW && <span style={{ fontSize:11,color:"var(--color-text-success)",fontWeight:500 }}>Vencedor ✓</span>}
                                </>
                              ) : (
                                <span style={{ fontSize:12,color:"var(--color-text-tertiary)",fontStyle:"italic",padding:"2px 0" }}>A definir</span>
                              )}
                            </div>
                          );
                        })}
        ...
        <div>
          <div style={{ background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"1.25rem",marginBottom:12 }}>
            <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between" }}>
              ...
              <button onClick={toggleUnlock} style={{
                background:results.knockoutUnlocked?"var(--color-background-success)":"var(--color-background-secondary)",
                color:results.knockoutUnlocked?"var(--color-text-success)":"var(--color-text-primary)",
                border:"0.5px solid var(--color-border-tertiary)",padding:"8px 18px",
                borderRadius:"var(--border-radius-md)",fontWeight:500,cursor:"pointer"
              }}>{results.knockoutUnlocked?"🔓 Bloquear":"🔒 Liberar"}</button>
            </div>
          </div>
          <p style={{ fontSize:13,color:"var(--color-text-secondary)",lineHeight:1.6,marginBottom:20 }}>
            ...
            3. Conforme os resultados saírem, registre os vencedores em "Resultados por fase".<br/>
          ...
          <div style={{ borderTop:"0.5px solid var(--color-border-tertiary)",paddingTop:20 }}>
            <p style={{ margin:"0 0 8px",fontWeight:500,fontSize:14,color:"var(--color-text-danger)" }}>⚠️ Zona de perigo</p>
            <p style={{ margin:"0 0 12px",fontSize:13,color:"var(--color-text-secondary)" }}>
              Apaga todos os participantes e palpites. Use apenas para limpar dados de teste antes do bolão começar de verdade.
            </p>
            <button onClick={()=>{
              if(window.confirm("Tem certeza? Isso apaga TODOS os participantes e palpites. Não tem como desfazer.")) {
                localStorage.removeItem("parts2026");
                window.location.reload();
              }
            }} style={{
              background:"var(--color-background-danger)",color:"var(--color-text-danger)",
              border:"0.5px solid var(--color-border-danger)",padding:"8px 18px",
              borderRadius:"var(--border-radius-md)",fontWeight:500,cursor:"pointer"
            }}>
              🗑️ Apagar todos os participantes
            </button>
          </div>
        </div>
      )}
...
// ── guesses screen ────────────────────────────────────────────────────────────
function GuessesScreen({ user, p, admin, onBrazil, onGroup, onKO, onSave, saveStatus, onBack }) {
  ...
  return (
    <div style={{ maxWidth:720,margin:"0 auto",padding:"1.5rem 1rem" }}>
      <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:"1.5rem" }}>
        ...
        <button onClick={onSave} style={{ marginLeft:"auto",display:"flex",alignItems:"center",gap:6,padding:"7px 16px" }}>
      ...
      <TabBar
        ...
        active={tab} onChange={setTab}
      />
      {tab==="brasil" && <BrazilTab p={p} admin={admin} onBrazil={onBrazil} />}
      {tab==="grupos" && <GroupsTab p={p} admin={admin} onGroup={onGroup} />}
      {tab==="matamata" && <KOTab p={p} admin={admin} onKO={onKO} />}
    </div>
  );
}

// ── brazil tab ────────────────────────────────────────────────────────────────
function BrazilTab({ p, admin, onBrazil }) {
  return (
    <div>
      ...
      {BRAZIL_GAMES.map(game=>{
        ...
        const result=admin.brazil[game.id]||{};
        ...
        return (
          <div key={game.id} style={{ background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"1rem 1.25rem",marginBottom:10 }}>
            ...
            <div style={{ display:"flex",alignItems:"center",gap:8,flexWrap:"wrap" }}>
              ...
              <span style={{ fontSize:14,minWidth:60 }}>{home}</span>
              ...
              <input type="number" min="0" max="20" value={guess.awayGoals}
                ...
                style={{ width:52,textAlign:"center",fontSize:20,fontWeight:500 }} placeholder="0" />
              <span style={{ fontSize:14,minWidth:60 }}>{away}</span>
...
// ── groups tab ────────────────────────────────────────────────────────────────
function GroupsTab({ p, admin, onGroup }) {
  return (
    <div>
      <div style={{ background:"var(--color-background-secondary)",borderRadius:"var(--border-radius-md)",padding:"10px 14px",marginBottom:14 }}>
        <p style={{ margin:0,fontSize:12,color:"var(--color-text-secondary)" }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(270px,1fr))",gap:10 }}>
        {GROUPS.map(group=>{
          ...
          const result=admin.groups[group.id]||{};
...
// ── knockout tab (participant) ────────────────────────────────────────────────
function KOTab({ p, admin, onKO }) {
  const [activeRound, setActiveRound] = useState("r32");
  const unlocked = admin.knockoutUnlocked;
  const koGuesses = p.knockout || {};
  const adminKO = admin.knockout || {};
  ...
  // teams entering each round (from user's prior picks, or r32 entrants)
    ...
    <div>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:12 }}>
        ...
        <span style={{ fontSize:12,color:"var(--color-text-secondary)",marginLeft:"auto" }}>
          {picks.filter(Boolean).length}/{numM} preenchidos
        </span>
      </div>
        ...
        {Array.from({length:numM},(_,mi)=>{
          ...
          return (
            <div key={mi} style={{
              ...
              border:correct?"1.5px solid var(--color-border-success)":wrong?"1.5px solid var(--color-border-danger)":"0.5px solid var(--color-border-tertiary)",
              ...
              {[t1,t2].map((team,ti)=>{
                ...
                const isO=official===team;
                const bg=COLORS[team]||"#888";
                return (
                  <div key={ti} onClick={()=>team&&t1&&t2&&onKO(activeRound,mi,team)} style={{
                    display:"flex",alignItems:"center",gap:8,padding:"8px 10px",
                    cursor:team&&t1&&t2?"pointer":"default",
                    background:isP?bg+"1a":"transparent",
                    borderTop:ti>0?"0.5px solid var(--color-border-tertiary)":"none",
                    transition:"background 0.12s",
                    opacity:!team?0.4:1
                  }}>
                    ...
                    <span style={{ fontSize:12,flex:1,fontWeight:isP?500:400,lineHeight:1.2 }}>
                      {team||"A definir"}
                    </span>
...
// ── ranking ───────────────────────────────────────────────────────────────────
function RankingScreen({ sorted, getTotal, admin, parts, onBack, onSelect }) {
  return (
    <div style={{ maxWidth:600,margin:"0 auto",padding:"1.5rem 1rem" }}>
      ...
      {sorted.map((name,i)=>{
        ...
        const brPts=BRAZIL_GAMES.reduce((a,g)=>{ const x=calcBrazilPts(p.brazil?.[g.id]||{},admin.brazil?.[g.id]); return a+(x||0); },0);
        const grPts=GROUPS.reduce((a,g)=>{ const x=calcGroupPts(p.groups?.[g.id]||{},admin.groups?.[g.id]); return a+(x||0); },0);
        const koPts=calcKOPts(p.knockout,admin.knockout);
        const medal=i===0?"🥇":i===1?"🥈":i===2?"🥉":null;
        return (
          <div key={name} onClick={()=>onSelect(name)} style={{ cursor:"pointer",background:"var(--color-background-primary)",border:"0.5px solid var(--color-border-tertiary)",borderRadius:"var(--border-radius-lg)",padding:"1rem 1.25rem",marginBottom:8,display:"flex",alignItems:"center",gap:12 }}>
            <span style={{ width:28,textAlign:"center",fontSize:i<3?18:14,color:"var(--color-text-secondary)" }}>{medal||`#${i+1}`}</span>
            <div style={{ flex:1 }}>
              <p style={{ margin:0,fontWeight:500,fontSize:15 }}>{name}</p>
              <p style={{ margin:"2px 0 0",fontSize:11,color:"var(--color-text-secondary)" }}>
                Brasil: {brPts}pts · Grupos: {grPts}pts · Mata-mata: {koPts}pts
              </p>
            </div>
            <span style={{ fontSize:20,fontWeight:500 }}>{total}</span>
            <i className="ti ti-chevron-right" style={{ color:"var(--color-text-tertiary)",fontSize:16 }} />
          </div>
        );
      })}
    </div>
